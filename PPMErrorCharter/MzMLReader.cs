...
#pragma warning restore 414
        ...
        public void ReadSpectraData(List<IdentData> data)
        {
            foreach (var datum in data)
            {
                SpectrumData spectrumData;
                if (!string.IsNullOrWhiteSpace(datum.NativeId))
                {
                    spectrumData = ReadMassSpectrumRandom(datum.NativeId);
                }
                else
                {
                    spectrumData = ReadMassSpectrumRandom(datum.ScanIdInt);
                }
                ...
                if (Math.Abs(datum.ExperMzRefined) < float.Epsilon)
                {
                    ...
                    {
                        datum.ExperMzRefined = spectrumData.Precursors[0].Ions[0].SelectedIonMz;
                    }
                }

                if (datum.ScanTimeSeconds <= 0)
                {
                    // StartTime is stored in minutes, we've been using seconds.
                    datum.ScanTimeSeconds = spectrumData.Scans[0].StartTime * 60;
                }
            }
        }
        ...
        */
        ...
        /// </summary>
        /// <param name="index"></param>
        private SpectrumData ReadMassSpectrumRandom(long index)
        {
            if (!_haveIndex || !_haveMetaData)
            {
                ReadMzMl();
        ...
        private SpectrumData ReadMassSpectrumRandom(string nativeId)
        {
            if (!_haveIndex || !_haveMetaData)
            {
                ReadMzMl();
            ...
            if (!_spectrumOffsets.OffsetsMapNative.ContainsKey(nativeId))
            {
                return null;
            }

            _fileReader.DiscardBufferedData();
            _fileReader.BaseStream.Position = _spectrumOffsets.OffsetsMapNative[nativeId];
            // Not allowed for a GZipStream.....
            using (var reader = XmlReader.Create(_fileReader, _xSettings))
            {
                reader.MoveToContent();
        ...
        private SpectrumData ReadSpectrum(XmlReader reader)
        {
            reader.MoveToContent();
            ...
            while (reader.ReadState == ReadState.Interactive)
            {
                ...
                switch (reader.Name)
                {
                    ...
                    //    // Schema requirements: one instance of this element
                    //    ReadSpectrumDescription(reader.ReadSubtree(), ref data.Scans, ref data.Precursors, out centroided);
